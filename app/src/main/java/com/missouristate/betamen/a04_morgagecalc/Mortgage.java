...
public class Mortgage {
    ...
    public Mortgage( ) {                                                //setter with defoult values
        setAmount( 100000.0f );
        setYears( 30 );
        setRate( 0.035f );
    }
    public Mortgage(Context context){                                                               //read any data if previously was recorded
        SharedPreferences pref = PreferenceManager.getDefaultSharedPreferences(context);
        setAmount(pref.getFloat(PREFERENCE_AMOUNT,100000.00f));                                  //set ammount, if it is empty- first time we run app - use those numbers as defoult
    ...
    public float monthlyPayment( ) {
        ...
        double temp = pow( 1/( 1 + mRate ), years * 12 );
    ...
    public void writeToFile( Context context ) {                                                 //write data to the file before closing
